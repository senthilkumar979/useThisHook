...
export const useStableCallbackApi: HookApi = {
  ...
  arguments: [
    { name: 'fn', type: 'T extends (...args) => unknown', description: 'The function you would normally recreate every render.' },
  ],
  returns: {
    type: 'T',
...
export const useResetStateApi: HookApi = {
  ...
  arguments: [
    { name: 'source', type: 'unknown', description: 'Identity of the thing being edited. Changing it resets state.' },
    { name: 'initialValue', type: 'T', description: 'State to use on first render and whenever source changes.' },
  ],
  returns: {
    type: '[state, setState]',
    ...
    fields: [
      ...
      { name: 'setState', type: 'Dispatch<SetStateAction<T>>', description: 'Update state until source changes.' },
    ],
  },
};

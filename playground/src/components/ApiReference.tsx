...
export const ApiReference = ({ name, api }: ApiReferenceProps) => (
  <div className="overflow-hidden rounded-2xl border border-line bg-surface">
    <div className="border-b border-line bg-gradient-to-r from-violet-500/10 via-transparent to-sky-500/10 px-5 py-4">
      <p className="text-[0.7rem] font-medium uppercase tracking-[0.16em] text-muted">Call signature</p>
    ...
    <div className="space-y-8 p-5">
      ...
      <div>
        ...
        <p className="mb-3 font-mono text-[0.8rem] text-violet-600 dark:text-sky-200/90">{api.returns.type}</p>
        <FieldTable
      ...
      {api.caveats && api.caveats.length > 0 && (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4">
          <h3 className="mb-2 text-sm font-semibold text-amber-900 dark:text-amber-100">Good to know</h3>
